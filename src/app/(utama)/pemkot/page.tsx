      ...
      <div className="grid gap-5 lg:grid-cols-[1.35fr_1fr]">
        <section className="flex flex-col gap-3">
          ...
          <CatatanDemo>
            ...
            sementara Sematang Borang dan Ilir Timur III paling sedikit. Peta
            seperti ini yang menjadi dasar menempatkan pembinaan, bukan
            pemerataan buta.
          </CatatanDemo>
        </section>
          ...
          <table className="w-full min-w-[640px] text-left">
            <thead>
              <tr className="border-b border-white/10">
                {["Event", "Penyelenggara", "Tanggal", "Peserta", "Status"].map(
                  (h) => (
                    <th
                      key={h}
              ...
              {EVENTS.map((e) => (
                ...
                >
                  <td className="px-4 py-3 text-[13px] font-semibold text-paper">
                    <Link href={`/event/${e.id}`} className="hover:text-aksen">
                      {e.nama}
                    </Link>
                  </td>
