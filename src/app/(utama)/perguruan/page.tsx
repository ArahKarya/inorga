...
export default function HalamanPerguruan() {
  ...
  const belumLengkap = anggota.filter(
    (a) => a.statusVerifikasi !== "terverifikasi",
  );
  const perluWali = anggota.filter(
    (a) => a.umur < 17 && !a.wali?.terverifikasi,
  );
  ...
  const peraih = anggota
    .map((a) => ({
      a,
      n: HASIL.filter((h) => h.atletId === a.id && h.medali).length,
    }))
    .filter((x) => x.n > 0)
    .sort((x, y) => y.n - x.n);
  const perKecamatan = KECAMATAN.map((k) => ({
    k,
    n: anggota.filter((a) => a.kecamatanId === k.id).length,
  }))
    .filter((x) => x.n > 0)
    .sort((x, y) => y.n - x.n);
  const perKelompok = (["Pra-Remaja", "Remaja", "Dewasa"] as const).map(
    (g) => ({ g, n: anggota.filter((a) => kelompokUmur(a.umur) === g).length }),
  );
  const pendaftarAktif = PENDAFTARAN.filter(
    (p) => ids.has(p.atletId) && eventById(p.eventId)?.status !== "selesai",
  );
    ...
    <main className="flex flex-col gap-10 pb-12">
      <header className="relative min-h-[260px] border-b border-white/10">
        ...
        <div className="inner relative flex min-h-[260px] flex-col justify-end gap-2 py-10">
          ...
          <h1 className="judul text-[clamp(30px,5vw,56px)] text-paper">
            {perguruan.nama}
          </h1>
          <p className="text-[13px] text-paper-dim">
            Kec. {namaKecamatan(perguruan.kecamatanId)} · berdiri{" "}
            {perguruan.berdiri} ·{" "}
            {perguruan.jenis === "nasional"
              ? "perguruan nasional"
              : "padepokan lokal"}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-10">
          <div className="grid grid-cols-3 gap-px border border-white/10 bg-white/10">
            <div className="bg-ink-700 p-4">
              <Statistik
                kecil
                angka={perguruan.jumlahAnggota}
                label="Anggota terdaftar"
                ket={`${anggota.length} pada data contoh`}
              />
            </div>
            <div className="bg-ink-700 p-4">
              <Statistik
                kecil
                angka={belumLengkap.length}
            ...
            <div className="bg-ink-700 p-4">
              <Statistik
                kecil
                angka={perluWali.length}
                label="Butuh izin wali"
              />
            </div>
          </div>
          ...
          <section className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-3 gap-px border border-white/10 bg-white/10">
              {[
                ["Emas", medali.emas, "text-emas"],
                ["Perak", medali.perak, "text-perak"],
                ["Perunggu", medali.perunggu, "text-perunggu"],
              ].map(([l, n, c]) => (
                <div
                  key={String(l)}
                ...
                >
                  ...
                  <span
                    className={`tnum font-display text-[32px] leading-none font-black ${c}`}
                  >
                    {n}
                  </span>
                </div>
            ...
            <Kartu className="divide-y divide-white/10">
              {peraih.map(({ a, n }) => (
                <Link
                  key={a.id}
                  href={`/atlet/${a.id}`}
                  className="flex items-center justify-between gap-3 px-4 py-3 transition-colors hover:bg-ink-600"
                >
                  <span className="text-[13px] text-paper">{a.nama}</span>
                  <span className="tnum font-mono text-[11px] text-muted">
                    {n} medali · {kelompokUmur(a.umur)}
                  </span>
                </Link>
              ))}
              {peraih.length === 0 ? (
                <p className="p-4 text-[13px] text-muted">
                  Belum ada medali bersistem.
                </p>
              ) : null}
            </Kartu>
          </section>
          ...
          <section className="flex flex-col gap-4">
            ...
            <Kartu className="flex flex-col gap-3 p-4">
              {perKecamatan.map(({ k, n }) => (
                <div key={k.id} className="flex flex-col gap-1.5">
                  ...
                  <div className="h-1.5 bg-ink-500">
                    <div
                      className="h-full bg-aksen"
                      style={{ width: `${(n / anggota.length) * 100}%` }}
                    />
                  </div>
                </div>
              ))}
            ...
            <CatatanDemo>
              Anggota datang dari {perKecamatan.length} kecamatan — jangkauan
              perguruan melampaui kecamatan kedudukannya. Ini yang dibaca Pemkot
              saat menempatkan pembinaan baru.
            </CatatanDemo>
          </section>
        </div>
        ...
        <div className="flex flex-col gap-10">
          <section className="flex flex-col gap-4">
            ...
            <Kartu className="flex flex-col gap-4 p-6">
              <p className="text-[13px] leading-relaxed text-paper-dim">
                Pilih beberapa anggota sekaligus untuk didaftarkan ke Palembang
                Open Silat Fest 2026. Data profil masing-masing terisi otomatis;
                pengurus hanya memilih kategori lomba.
              </p>
              <button type="button" className={TOMBOL}>
                Pilih anggota &amp; daftarkan
              </button>
              <CatatanDemo>
                Jalur ini juga yang dipakai untuk anggota yang tidak punya
                ponsel sendiri. Persetujuan wali tetap direkam terpisah dan
                tidak bisa diwakilkan pengurus.
              </CatatanDemo>
            </Kartu>
          </section>
            ...
            <Kartu className="divide-y divide-white/10">
              {anggota.map((a) => (
                <Link
                  key={a.id}
                  href={`/atlet/${a.id}`}
                  className="flex items-center gap-3 p-4 transition-colors hover:bg-ink-600 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-aksen"
                >
                  ...
                  <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                    ...
                    <p className="font-mono text-[11px] text-muted">
                      ...
                      {namaKecamatan(a.kecamatanId)}
                    </p>
                  </div>
                  {a.umur < 17 && !a.wali?.terverifikasi ? (
                    <Lencana nada="kuning">Wali belum</Lencana>
                  ) : null}
          ...
          <section className="flex flex-col gap-4">
            <JudulBagian
              ...
              aksi={
                <span className="tnum font-mono text-[11px] text-muted">
                  {pendaftarAktif.length} pendaftaran
                </span>
              }
            />
            <Kartu className="divide-y divide-white/10">
                  ...
                  <div key={p.id} className="flex items-center gap-3 px-4 py-3">
                    ...
                    <div className="flex min-w-0 flex-1 flex-col">
                      ...
                      <span className="truncate font-mono text-[10px] text-muted">
                        {ev.nama} · {labelKategori(p.kategoriId)}
                      </span>
                    </div>
                    <Lencana
                      nada={ev.status === "berlangsung" ? "cyan" : "netral"}
                    >
      ...
      <p className="inner font-mono text-[10px] tracking-[0.1em] text-muted uppercase">
        {EVENTS.length} event tercatat · data contoh
      </p>
    </main>
  );
}
